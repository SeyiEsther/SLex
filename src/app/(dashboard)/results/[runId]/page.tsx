...
export default async function ResultsPage({ params }: PageProps) {
  ...
  // Collect unique filter options from the issues themselves
  ...
  return (
    <div>
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            ...
            {run.payroll_filename && (
              <> · <span className="font-mono text-xs">{run.payroll_filename}</span></>
            )}
            {run.benefits_filename && (
              <> &amp; <span className="font-mono text-xs">{run.benefits_filename}</span></>
            )}
          </p>
        </div>
        <Link href="/upload" className="text-sm text-indigo-600 hover:underline font-medium shrink-0">
          + New run
        </Link>
      </div>
      ...
      {issues.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm px-6 py-4 mb-6">
          <div className="flex items-center gap-6 flex-wrap text-xs">
            <span className="font-medium text-gray-500 uppercase tracking-wide">Breakdown</span>
            {criticalCount > 0 && (
      ...
      {/* Issues */}
      <h2 className="text-base font-semibold text-gray-900 mb-3">Issues Found</h2>
